...
##################################################
#                                                #
# Purpose: Install and configure live migration  #
...
while read  -r line
 do
    ...
    controller_pub_nic=$(ssh "$controller" 'cat /root/.nic_interfaces'| awk '/public_interface/ {print $3}')
    # Get the name of ip of public network
    controller_pub_ip=$(ssh "$controller" ip addr show $controller_pub_nic| awk '/inet / {print $2}'| sed 's/\/.*//')
 fi
 done < /etc/hosts

# Edit /etc/fstab file
cat >> /etc/fstab  <<EOF
$controller_pub_ip:/ /var/lib/nova/instances nfs4 defaults 0 0
EOF
...
sed -i -e 's/nova:x:106:113::\/var\/lib\/nova:\/bin\/false/nova:x:106:113::\/var\/lib\/nova:\/bin\/sh/g' /etc/passwd
chsh -s /bin/sh nova
...
echo "##############################################################################################"
echo;
echo "Live migration setup in "$compute_name" node is done."
echo;
echo "##############################################################################################"
echo;